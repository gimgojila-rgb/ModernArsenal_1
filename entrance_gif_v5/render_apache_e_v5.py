"""AH-64E Guardian entrance preview, v5 (v4 with the real MQ-1C / RQ-7B sprites and lights, puff smoke, a back-blast cloud).
...
Shot list (different from the D's dive-in):
  ...
  2. Spike NLOS off the launcher from stand-off range. The camera rides with the missile: it leads the missile through
     the loft and over the top (the launch itself is captured every tick, so the wings swinging out after the missile
     clears the canister play in slow motion), then cuts to the Spike's seeker feed (EO, stepped FOV) for the terminal dive, and it
     whites out on impact.
  3. The explosion from the side, then a whip pan back to the Guardian. An MQ-1C (half the Apache) and an RQ-7B (about 1/5,
     drawn 1:1) sweep past it from right to left, one over the rotor and one under the skids. Title card.
...
def cloud(x, y, vx, vy, life, s0, s1, alpha, col):
...
def step():
    ...
    for p in parts:
        ...
        if p[8] == 'cloud': p[2] = p[2] * 0.95 + 0.004; p[3] = p[3] * 0.95 - 0.012
...
def spike_step(t):
    if t - LAUNCH == 0:
        ...
        for _ in range(8):                                             # gas-generator puff, blown back off the tube
            puff(m.p[0] + 6, m.p[1], rng.uniform(0.5, 3), rng.uniform(-0.8, 0.6), 30, rng.uniform(3, 6), (200, 196, 186), 'smoke')
        launch_blast(m.p)
    ...
    if m.phase == 'eject':
        ...
        for k in range(3):
            ...
            cloud(q[0] + rng.uniform(-1, 1), q[1] + rng.uniform(-1, 1), rng.uniform(-0.2, 0.2) - m.v[0] * 0.02, rng.uniform(-0.3, 0.05) - m.v[1] * 0.02,
                  rng.randint(110, 240), rng.uniform(3.5, 5), rng.uniform(13, 22), 0.5, (200, 198, 192))
    m.p = add(m.p, m.v)
    if m.phase == 'dive' and st['seeker'] is None and math.hypot(m.p[0] - PLAYER[0], m.p[1] - PLAYER[1]) < 640:
        st['seeker'] = t
