# Modern Arsenal 쇼릴 (76초)

`../ModernArsenal_Showreel.mp4`를 만든 코드예요. 1920×1080, 60fps, 76초이고, 음악은 "Low Altitude Assault"(120BPM)예요.
...
- `mix.py`: 곡 0~64.06초와 168.06초~끝을 이어 붙이고, 합성 효과음이랑 모드 사운드(드론 통과음, 험비 엔진, M2)를 섞어요
...
`assets/ApacheDBoss.png`, `ApacheDBoss_Radar.png`는 `preview_D_vs_E.png`에서 E 스프라이트 위치(2배 스케일, 좌표 120,660, D는 516px 위)에 맞춰 떼어낸 AH-64D 몸체와 레이더 돔이에요.
