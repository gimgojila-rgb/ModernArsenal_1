...
SX, SY = 17.0, 12.5      # squat on purpose: a true-proportion side view reads too spindly in game
S = SX
...
def cells(poly):
    im = Image.new('L', (int(W * SX), int(H * SY * 2)), 0)
    ImageDraw.Draw(im).polygon([(x - X0, (y - Y0) * 2) for x, y in poly], fill=255)
    m = np.asarray(im.resize((W * 17, H * 25))).astype(np.float32) / 255
    return m.reshape(H, 25, W, 17).mean(axis=(1, 3))
...
for x in range(bx0, bx1 + 1):
    tip = x >= bx1 - 2
    rows = [(by0, G[0]), (by0 + 1, G[8]), (by0 + 2, G[7]), (by0 + 3, G[5]), (by0 + 4, G[0])] if not tip else [(by0 + 1, G[0]), (by0 + 2, G[0]), (by0 + 3, G[0])]
    if x == bx1:
        rows = [(by0 + 2, G[0])]
    if x == bx1 - 1:
        rows = [(by0 + 1, G[0]), (by0 + 2, G[6]), (by0 + 3, G[0])]
    for (y, col) in rows:
...
for bx in (1305, 1595):
    x = cx(bx)
    for xx in (x, x + 1):
        put(by0 + 1, xx, (214, 64, 54)); put(by0 + 2, xx, (196, 48, 40)); put(by0 + 3, xx, RED)
...
for n in ('wheel', 'wheelfar', 'nosewheel', 'ball'):
    for y in range(H):
        for x in range(W):
            if label[y, x] == n:
                img[y, x] = 0
disc(1085, 790, 3.0, DARK[0], DARK[0])
disc(1075, 868, 3.6, DARK[1], G[0], G[5])
disc(480, 860, 2.4, DARK[1], G[0], G[5])
disc(852, 752, 3.1, G[5], G[0])
...
OUT = '/home/claude/shadow/out/'
import os; os.makedirs(OUT, exist_ok=True)
def save2(arr, name): Image.fromarray(arr).resize((W * 2, H * 2), Image.NEAREST).save(OUT + name)
Image.fromarray(img).save(OUT + 'shadow_1x.png')
save2(img, 'RQ7Shadow.png'); save2(slime, 'RQ7Shadow_Slime.png'); save2(marksL, 'RQ7Shadow_MarksL.png'); save2(marksR, 'RQ7Shadow_MarksR.png')
def off2(p): return [p[0] * 2 + 1 - W, p[1] * 2 + 1 - H]
PROP = (cx(1352), cy(632))
json.dump({k: ([off2(p) for p in v] if isinstance(v, list) else off2(v)) for k, v in FIT.items()} | {'prop_axis': off2(PROP)},
          open(OUT + 'RQ7Shadow_lights.json', 'w'), indent=1)
...
Image.fromarray(sheet).resize((6, sheet.shape[0] * 2), Image.NEAREST).save(OUT + 'RQ7Shadow_Prop.png')
...
Image.fromarray(blur).resize((10, FH * 2), Image.NEAREST).save(OUT + 'RQ7Shadow_PropBlur.png')
