...
body=Image.open('out/RQ7Shadow.png').convert('RGBA')
slime=np.asarray(Image.open('out/RQ7Shadow_Slime.png').convert('RGBA'))[...,3].astype(np.float32)/255
prop=Image.open('out/RQ7Shadow_Prop.png').convert('RGBA'); blur=Image.open('out/RQ7Shadow_PropBlur.png').convert('RGBA')
L=json.load(open('out/RQ7Shadow_lights.json'))
...
def frame(tick, face_left):
    ...
    b.alpha_composite(Image.open('out/RQ7Shadow_MarksL.png' if face_left else 'out/RQ7Shadow_MarksR.png').convert('RGBA'))
    ax=P(L['prop_axis'])
...
frame(10,True).save('/home/claude/shadow/left.png')
frame(0,True).save('/home/claude/shadow/strobe.png')
...
q[0].save('/mnt/user-data/outputs/rq7_shadow_idle.gif',save_all=True,append_images=q[1:],duration=33,loop=0,disposal=1)
frame(10,False).save('/home/claude/shadow/right.png')
